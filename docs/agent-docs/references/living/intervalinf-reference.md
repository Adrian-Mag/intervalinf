...
- `full_spectrum_utils.py` (**Phase 1**, new): `BlockIndex` (frozen dataclass with fields `s` and `t`), `enumerate_blocks(reg, s_max)` (returns sorted list of non-empty `BlockIndex` for even `s ≤ s_max`; `t` is a real-harmonic component index `0 ≤ t ≤ 2s`), `block_data_split(reg, blocks)` (returns dict mapping each block to its sub-registry).
- `tests/test_full_spectrum_utils.py` (**Phase 1**, new): three pytest tests for `full_spectrum_utils` block indexing (parity/range, skip-empty, partition).
- `example.ipynb` (**Phase 1**, new): spectral inventory notebook — loads registry filtered to kernel-catalog modes, calls `enumerate_blocks` with `s_max=4`, prints block sizes, and produces a bar plot of data count per `(s,t)` block grouped by degree `s`.
