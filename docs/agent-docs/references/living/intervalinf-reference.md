...
## Rough Work / Standalone Scripts (`rough_work/`)

Scripts in this directory are not part of the package but provide standalone experiments and benchmarks.

| Script | Purpose |
|---|---|
| `rough_work/bg_dli_1d_sweep.py` | **BG vs DLI 1D admissible-interval comparison script.** Sweeps $N_p=1$ admissible interval widths for both BG and DLI across $N_d \in \{5, 10, 20, 40\}$, 3 `NormalModesProvider` forward-operator seeds, and 3 data-noise seeds (36 cases total, ~110 s). Forward operators are nested across $N_d$ (same seed → `N_d=10` inherits the first 10 kernels from `N_d=5`). Data-confidence set is chi-square calibrated (`EllipsoidSupportFunction`). Key builders: `build_problem`, `compute_dli_interval` (proximal bundle method via `DualMasterCostFunction`), `compute_bg_interval` (Minkowski-sum support algebra). Gap metrics: `width_ratio`, `excess_width`, `hausdorff_gap`, `containment_violation`. Outputs: raw CSV and summary-by-$N_d$ CSV in `bg_dli_1d_sweep_results/`; width-ratio and absolute-width PNG/PDF plots in `bg_dli_1d_sweep_figures/`. Usage: `conda run -n inferences3 python intervalinf/rough_work/bg_dli_1d_sweep.py` |
