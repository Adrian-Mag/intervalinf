...
- `example.ipynb` (**Phases 1–6**, updated): spectral inventory → data visualisation → forward operators → priors → posteriors → property operators + per-block property vector printout → property posterior assembly ($\mu_P$, $C_P$), mean±std per target, correlation heatmap.
