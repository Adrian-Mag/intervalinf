...
- `synthetic_vp_vs_dli.ipynb`: synthetic deterministic linear inference notebook designed as a midpoint between `dli.ipynb` and `realistic_dli.ipynb`. Uses a two-block direct-sum model space `M_vp ⊕ M_vs` with basis-free `Lebesgue` blocks on `[0,1]`, synthetic `NormalModesProvider` kernels for both forward blocks, and a block forward operator `G = [G_vp, G_vs]` assembled with `RowLinearOperator`. The property operator remains intentionally simple: two bump-function targets on the `vp` block with a null `vs` contribution, so the DLI solve stays readable while still exercising direct-sum structure. Synthetic truth is generated by seeded cosine-series `Function` objects for both `vp` and `vs`, data are corrupted with homoscedastic Gaussian noise, and both the model prior set and data-confidence set are simple `BallSupportFunction` objects rather than ellipsoids. The notebook ends with a standard `DualMasterCostFunction` + `ProximalBundleMethod` solve in the `±e_i` directions, followed by prior-vs-posterior bound plots and reduction-factor summaries.
- `synthetic_vp_vs_ellipsoid_dli.ipynb`: **ellipsoidal DLI extension** of `synthetic_vp_vs_dli.ipynb` — **COMPLETE**. Replaces both the model prior and data-confidence sets with ellipsoids. The model space is the same two-block direct sum `M_vp ⊕ M_vs` with basis-free `Lebesgue` blocks on `[0,1]`, and the synthetic forward structure (`NormalModesProvider`, `SOLAOperator`, `RowLinearOperator`) is identical to `synthetic_vp_vs_dli.ipynb`. The key difference is the **operator-theoretic model prior**: a Bessel-Sobolev confidence ellipsoid `E = {m : ⟨C₀⁻¹m, m⟩_M ≤ χ²(dof_eff, 0.95)}` built from `BesselSobolevInverse` covariance blocks, `BesselSobolev` shape blocks, and `BlockDiagonalLinearOperator` block assembly — no dense covariance matrices and no `.matrix(...)` discretisation. **Operator convention**: `BesselSobolevInverse(k, s, L)` scales the $i$-th mode by $(k^2 + \lambda_i)^{-s/2}$ where $\lambda_i$ are eigenvalues of `L = alpha*(-Delta) ≥ 0`; `BesselSobolev(k, s, L)` scales by $(k^2 + \lambda_i)^{s/2}$. Per-block Matérn-like hyperparameters: vp (`s=3, L_corr=0.2, dc_variance=2`) and vs (`s=3, L_corr=0.15, dc_variance=2`) with Neumann boundary conditions. **Amplitude convention**: `k = dc_variance^{-1/s}` so the DC (zero-mode) eigenvalue of `C_0` equals `dc_variance` exactly; with `alpha = L_corr^2 * k^2` the correlation shape `(1 + L_corr^2 * lambda_i(-Delta))^{-s/2}` is independent of `dc_variance`. Effective dof is determined at runtime by KL truncation via `KLSampler` (tol=1e-4, N=10 per block — exact value depends on the hyperparameter configuration). **Data constraint**: heteroscedastic diagonal data-confidence ellipsoid `V = {η : Σ_i (η_i/σ_{d,i})² ≤ χ²_{N_d}(0.95)}` built via three `DiagonalSparseMatrixLinearOperator` objects (precision `A_d=diag(1/σ_d²)`, covariance `C_d=diag(σ_d²)`, sqrt `C_d_sqrt=diag(σ_d)`) passed to `EllipsoidSupportFunction(D, center=0, radius=√χ²_{N_d}(0.95), ...)`. **DLI solve**: `DualMasterCostFunction` + `ProximalBundleMethod(tol=1e-4, max_iters=500, bundle_size=50)` for ±e_i directions (sequential, warm_start=True). **Results**: posterior half-widths ~0.51 and ~0.91 vs prior ~18.54 (~96% mean reduction); both true properties captured; posterior sanity `post_half ≤ prior_half + 1e-3` passes. No model `.matrix(...)` calls anywhere. **Narrative**: intro text explains ellipsoid vs ball trade-off; closing "Key Takeaways" section compares the two notebooks with a feature table and explains the operator-theoretic construction pipeline.
- `synthetic_vp_vs_zero_region_dli.ipynb`: extension of `synthetic_vp_vs_dli.ipynb` in which the true `vs` field is a seeded cosine series multiplied by a smooth tanh cutoff, making `vs` effectively zero on the upper half of the domain (`x > 0.5`). The forward operator remains the same two-block `RowLinearOperator([G_vp, G_vs])` built from separate `NormalModesProvider` instances, and the property map still targets two bump-function averages on the `vp` block with null `vs` contribution. This keeps the inverse problem readable while introducing support structure that is present in the truth but absent from the prior: the model prior and data-confidence sets are still simple `BallSupportFunction` norm balls, so any posterior tightening reflects data consistency rather than an explicit support prior. The notebook adds a dedicated true-model section highlighting the `vs` zero region and then runs the same `DualMasterCostFunction` + `ProximalBundleMethod` DLI solve with prior/posterior interval comparisons.
...
- `realistic_dli_audit.py`: standalone audit script kept separate from the notebooks while investigating discrepancies between `dli.ipynb` and `realistic_dli.ipynb`. **Phase 1** records the baseline formulation actually being solved, including `N_d`, `N_p`, support-function types, solver settings, and saved solve summaries, and explains why wall-clock time alone is a poor proxy for problem complexity. **Phase 2** (`run_phase2_instrumentation()`) performs a live reconstruction of the realistic_dli problem from scratch — building all model spaces, forward/property operators, data-error and model-prior support functions, and synthetic true model — then runs the proximal bundle method sequentially per property direction, recording per-direction solve time, bundle iterations, convergence flag, and ‖λ*‖₂ (the optimal dual variable norm). The key diagnostic question is whether λ* ≈ 0 (prior-dominated) or λ* ≠ 0 (data-constraining). With the real kernel catalog at N_d=5: +e_0 ‖λ*‖=0.0013 (reduction 1.03×), +e_1 ‖λ*‖=0.0218 (reduction 1.70×), -e_0 ‖λ*‖=0.0015 (reduction 1.03×), -e_1 ‖λ*‖=0.0302 (reduction 1.96×) — **verdict: DATA-CONSTRAINING** (mean 1.43×). A self-check asserts posterior ≤ prior for every direction. Phase 2 supports `USE_SYNTHETIC_FALLBACK` (automatic when catalog is absent) and provides explicit adjoint mappings for all scalar-block operators. **Phase 3** (`run_phase3_nd_sweep()`) sweeps `N_d ∈ [5, 10, 20, 30, 40, 50]` with `N_p=2` fixed, reusing model spaces / property operators / model prior (built once) and rebuilding only `D`, `G`, data-error support, and `d̃` per iteration. Same seeds (cosine coefficients via `RandomState(42/24/84/12)`; noise via `np.random.seed(42)` fresh per N_d) and same sigma rule (`σ_d = 0.10 × linspace(1, 2, N_d)`). **Key finding**: with the real catalog, N_d=5 and N_d=10 are data-constraining (mean_red 1.37× and 1.49×; max ‖λ*‖=0.030 and 0.008), but N_d ≥ 20 is **fully prior-dominated** (‖λ*‖=0, reduction=1.000× for all properties) — additional catalog modes past index ~10 carry no useful information for the bump-function properties at this prior/noise configuration. Solve time is non-monotone (N_d=5: 2.3 s/154 iters; N_d=10: 5.2 s/212 iters; N_d=20: 1.1 s/25 iters) because prior-dominated cases converge in very few iterations. All assertions posterior ≤ prior pass. **Phase 4** (`run_phase4_diagnosis()`) diagnoses and classifies the prior-dominated collapse. Step A (tight-noise sweep, σ_base=0.01 vs 0.10, N_d=[5,10,15,20,25,30]) finds the transition persists at N_d=15 regardless of noise level — ruling out SNR-dilution as the primary cause. Step B (mode geometry, N_d=30) computes cosine similarity |⟨g_k, b_i⟩| / (‖g_k‖ · ‖b_i‖) for each mode k and bump target i: **cos(b_0)=0 for ALL modes** (pure geometric dead-zone for p0); cos(b_1) decays from ~0.87 to ~0.32 monotonically. The `_VERDICT_EXPLANATIONS` dict in the module defines EXPECTED_GEOMETRY / SNR_DILUTION / BOTH / EXPERIMENT_DESIGN / NOTEBOOK_BUG narratives. **Verdict: EXPECTED_GEOMETRY** — the notebook is not buggy; the posterior recovers the prior at N_d≥15 because the chi² ball size and effective kernel rank together exceed what the property targets can leverage. A markdown audit note is inserted into `realistic_dli.ipynb` immediately before the `N_d = 50` code cell (cell index 8 after insertion, id=ac35c0b1) explaining the finding.
- `realistic_dli.ipynb`: **realistic DLI mirror** of `old_demos/paper_demos/example_1.ipynb`. Uses the same direct-sum model structure (`M_vp ⊕ M_vs ⊕ M_rho ⊕ M_σ₀ ⊕ M_σ₁`), the same `SensitivityKernelCatalog`/`SensitivityKernelProvider` operator construction pattern, and a chi-squared deterministic data error set — currently configured at `N_d=50`, `N_p=2`. The functional model blocks are intentionally **basis-free** (`dim=0`, `basis=None`) and the prior is built **operator-theoretically**, not by assembling dense covariance matrices: `BesselSobolevInverse` supplies covariance blocks, `BesselSobolev` supplies the shape/operator-inverse blocks, and nested `BlockDiagonalLinearOperator` objects assemble the full model prior. The data-noise model is an explicit **heteroscedastic diagonal covariance** `C_D = diag(σ_d^2)` driven by a configured per-datum standard-deviation vector rather than by signal amplitude. The model prior is a strict 0.95 ellipsoid $\mathcal{E} = \{m : \langle A m,m\rangle_\mathcal{M} \leq \chi^2_{d_\mathrm{eff}}(0.95)\}$ represented through `EllipsoidSupportFunction`, with effective dof computed by KL truncation (tol=1e-4, helper count N=10, final dof=42). No `.matrix(...)` discretization is used in the notebook’s prior construction. Requires the external kernel catalog `kernels_modeplotaat_Adrian` (set `INTERVALINF_KERNEL_CATALOG_DIR` env var); raises `FileNotFoundError` if the catalog is missing rather than silently substituting synthetic proxies. A clearly-labeled `USE_SYNTHETIC_FALLBACK` flag (default `False`) can be set for offline exploration.
...
**Last Updated:** 2026-03-12 (Phase 3 of `synthetic_vp_vs_ellipsoid_dli.ipynb` complete: notebook finalized as finished demo — removed phase-scaffold wording from intro and summary, added "Key Takeaways" closing section comparing ellipsoid vs ball constraints and explaining operator-theoretic construction pipeline; living reference entry updated to reflect complete status)
