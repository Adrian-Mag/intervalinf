...
- `full_spectrum_utils.py` (**Phase 1 + Phase 2**, updated): `BlockIndex` (frozen dataclass with fields `s` and `t`), `enumerate_blocks(reg, s_max)` (returns sorted list of non-empty `BlockIndex` for even `s ≤ s_max`; `t` is a real-harmonic component index `0 ≤ t ≤ 2s`), `block_data_split(reg, blocks)` (returns dict mapping each block to its sub-registry). **Phase 2 additions:** `RadialSpecs` (dataclass with fields `n_basis=100`, `earth_radius_km`, `icb_radius_km=1217.5`, `cmb_radius_km=3480.0`, `lebesgue_cfg`, `sola_cfg`, `parallel_cfg`; `__post_init__` fills None fields with sensible defaults), `make_radial_space(n, domain, *, weight=1.0, basis='ND', integration_config, parallel_config)` (returns `Lebesgue`; raises `NotImplementedError` for `weight != 1.0`), `build_block_forward(s, t, reg_st, catalog, specs)` (builds model space + SOLA forward operators for one (s,t) block; returns `(G_st, C_D_st, M_st, D_st)` where `G_st` is `RowLinearOperator`, `C_D_st` is `GaussianMeasure` with diagonal covariance, `M_st` is `HilbertSpaceDirectSum`, `D_st` is `EuclideanSpace`).
- `tests/test_full_spectrum_utils.py` (**Phase 1 + Phase 2**, updated): seven pytest tests — three for block indexing (parity/range, skip-empty, partition) plus four for Phase 2 (`test_make_radial_space_unit_weight_matches_lebesgue`, `test_make_radial_space_nonunit_weight_raises`, `test_block_forward_shapes`, `test_block_forward_noise_covariance_diagonal`). **API discovery:** `LinearOperator` vectors are applied with `op(x)` (callable interface), not `op @ x` (which composes operators).
- `example.ipynb` (**Phase 1 + Phase 2**, updated): spectral inventory (Phase 1) plus a Phase 2 cell that constructs `RadialSpecs(n_basis=100, parallel_cfg=ParallelConfig(enabled=True, n_jobs=12))`, builds `forward_dict` via `build_block_forward` for every block, and prints a summary of `(N_d, M.dim)` per block.
