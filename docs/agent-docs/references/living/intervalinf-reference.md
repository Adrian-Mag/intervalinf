...
The `intervalinf/demos/old_demos/sola_demos/` directory contains the historical SOLA notebooks plus a compact Bessel-Sobolev update.

- `sola_failure_bessel_sobolev.ipynb`: upgraded SOLA-failure demonstration that follows the historical `sola_failure.ipynb` narrative while keeping the workflow on `pygeoinf`/`intervalinf` abstractions. It first sets up a normal basis-free `Lebesgue(0, domain, basis=None)` interval problem, solves it with a unimodular SOLA estimator, shows that the normal true properties lie inside all `+/-2 std` bands, and plots resolving kernels against target kernels. It then defines alternative target values far outside every SOLA band, builds an exact Bessel covariance-factor constraint matrix by mapping `BesselSobolevInverse.spectral_factor()` modes through the `SOLAOperator` data/property maps, solves the small dense least-squares problem for a perturbation with zero data change and the requested property change, and finally reveals that swapping this model in as the true model leaves the SOLA estimate exactly unchanged while all true properties are outside the reported bands. The notebook also plots precision/covariance eigenvalues and selected Laplacian eigenfunctions to show which scales are cheap or expensive in the Bessel-Sobolev norm. The final cell asserts the normal-case success, hidden-model data fit, same-data SOLA invariance, and out-of-band alternative properties.

The `intervalinf/demos/old_demos/paper_demos/` directory contains paper-oriented comparison notebooks, legacy prototypes, and the full-spectrum splitting-function PLI example (in progress).

- Layout: shared data/model-building helpers live in `utils/`; standalone plotting/viewer modules live in `visualization/`. Visualization modules import `../utils/` through `visualization/_path_setup.py` so they can still be run directly as scripts.
- `visualization/normal_mode_data_viewer.py`: Matplotlib-widget explorer for the bundled normal-mode splitting dataset. It defaults `--data-root` to `old_demos/paper_demos/data` relative to the `visualization/` script directory's parent, parses observed `.new` coefficient/error files, synthetic `raw_*.raw` predictions, PREM-layer sensitivity kernels `kern_*_{vp,vs,rho,topo}_PREM-layers-all_PREM-iso.kern`, and `PREM_all-layers_radius`; supports interactive switching between observed data, reported errors, volumetric/topography kernels, and observed-vs-synthetic comparisons; also supports `--list`, `--save`, and `--no-show` for noninteractive checks, and raises a clear `SystemExit` if the synthetic catalog is missing or empty.
- `full_spectrum_utils.py` (**Phases 1–6**, updated): `BlockIndex` (frozen dataclass with fields `s` and `t`), `enumerate_blocks(reg, s_max)` (returns sorted list of non-empty `BlockIndex` for even `s ≤ s_max`; `t` is a real-harmonic component index `0 ≤ t ≤ 2s`), `block_data_split(reg, blocks)` (returns dict mapping each block to its sub-registry). **Phase 2 additions:** `RadialSpecs` (dataclass with fields `n_basis=100`, `earth_radius_km`, `icb_radius_km=1217.5`, `cmb_radius_km=3480.0`, `lebesgue_cfg`, `sola_cfg`, `parallel_cfg`; `__post_init__` fills None fields with sensible defaults), `make_radial_space(n, domain, *, weight=1.0, basis='ND', integration_config, parallel_config)` (returns `Lebesgue`; raises `NotImplementedError` for `weight != 1.0`), `build_block_forward(s, t, reg_st, catalog, specs)` (builds model space + SOLA forward operators for one (s,t) block; returns `(G_st, C_D_st, M_st, D_st)` where `G_st` is `RowLinearOperator`, `C_D_st` is `GaussianMeasure` with diagonal covariance, `M_st` is `HilbertSpaceDirectSum`, `D_st` is `EuclideanSpace`). **Phase 3 additions:** `build_shared_bessel_blocks(specs)` (flat mode: one `BesselSobolevInverse` per radial parameter; weighted mode: requires explicit ST degrees and builds degree-aware `(component, degree)` covariances with `ell=degree`, plus degree-zero aliases only when `0` is requested), `prior_power_spectrum_default(p, s)` (flat power spectrum), `build_block_prior(s, t, shared_bessel, specs, tau_fn, sigma_var)` (assembles `GaussianMeasure` prior). **Phase 4 additions:** `solve_block(s, t, G_st, C_D_st, prior_st, d_st)` (data-space `LinearBayesianInversion` + `LUSolver` for one block), `solve_all_blocks(forward_dict, prior_dict, split, n_jobs)` (serial for `n_jobs=1`; otherwise joblib process-parallel over blocks with per-worker native thread caps to avoid BLAS/FFT oversubscription). **Phase 5 additions:** `build_property_operator(targets, blocks, forward_dict, specs, s_max, n_radial)` (returns `dict{BlockIndex: LinearOperator}` mapping each block's model space `M_st → EuclideanSpace(N_p)`; targets are `BulkTarget`/`CMBTarget` from `property_targets.py`; `BulkTarget` now uses the separable mapping $T_i(m)=B_{st,i}\int a_i(r)f_{p,st}(r)r^2dr$ with unit-surface angular coefficients and unit-`r^2dr` radial normalization, while `CMBTarget` remains `B_{st,i}\sigma_1`; the adjoint returns the corresponding `r^2`-weighted radial factors as `_IFunction` callables via `np.interp` over a precomputed grid). **Phase 6 additions:** `assemble_property_posterior(property_op_dict, model_posterior_dict)` (assembles $\mathcal{N}(\mu_P, C_P)$ by accumulating mean $\mu_P = \sum_{st} T_{st}(\tilde{m}_{st})$ and covariance $C_P = \sum_{st} T_{st} C^{post}_{st} T^*_{st}$ column-by-column; symmetrizes $C_P$; returns `GaussianMeasure` on `EuclideanSpace(N_p)` with dense matrix covariance operator).
- `docs/agent-docs/completed-plans/general-ell-radial-laplacian-plan.md` (**complete**, 2026-06-06): implemented spectral `RadialLaplacian` support beyond `ell=0` using SciPy spherical Bessel functions and robust root finding, then wired weighted full-spectrum priors so each `(s,t)` block uses degree-aware radial covariance `ell=s` while flat priors remain unchanged.
- `docs/agent-docs/references/living/prior-calibration-explainer.md`: detailed plain-language explanation of the degree-aware prior calibration algorithm, including the unit-scale Monte Carlo, the degree decay `w_s=(1+s)^(-\beta)`, and the final `tau_{p,s}=a_p w_s` scaling rule.
- `docs/agent-docs/references/living/conversation-summary-2026-06-07.md`: comprehensive summary of the scientific and implementation discussion leading to the current degree-aware weighted prior, calibration workflow, tuner integration, and documentation state.
- `synthetic_data.py` / `prior_posterior_tuner_app.py` (**synthetic mode**, updated): `compute_least_norm(block, G_st, d_real, specs, *, C_ref=None, data_std=None, target_chi_rms=None)` still supports the old L2 exact least-norm path, but GUI synthetic mode now passes a fixed tau=1 Bessel-Sobolev reference covariance plus `data_std` and `target_chi_rms=2.0`. The smooth path builds `G C_ref G*` and uses `_solve_target_smooth_alpha(...)` to choose the largest positive regularization `lambda` in `(G C_ref G* + lambda C_D) alpha = d` whose RMS standardized residual is below target; if the target is unreachable, it returns the best finite smooth fit and marks `fit_target_met=False`. `SynthLeastNorm` stores `fit_mode`, `fit_lambda`, `fit_chi_rms`, `fit_chi2_per_datum`, and target metadata for the GUI label. This avoids the numerically unstable exact smooth solve while keeping the synthetic true model independent of tunable prior amplitudes. `tests/test_synthetic_data.py` covers feasible and unreachable target selection for the pure NumPy helper.
- `prior_posterior_tuner_app.py` / `prior_posterior_tuner_remote.py` (**2026-05-28 noise-scaling update**): the Tkinter tuner now exposes a global `data noise × σ_D` control. Local and europa single-block posterior solves rebuild the assumed diagonal data-noise measure from `noise_multiplier * error_vector` before calling `solve_block(...)`, while synthetic data generation continues to use its separate `noise_scale` control. The posterior data-fit panel uses the same assumed noise level for error bars and RMS standardized residual.
- `prior_posterior_tuner_app.py` (**2026-05-29 Bayesian-inference panel**): added a "Bayesian inference (pushforward)" section with two buttons that leave the single-block inversion workflow untouched. `🎯 Show Target Kernels` builds a `target_kernel_viz.TargetKernelViewer` data engine (default targets via `_make_default_targets(specs, s_max)`) in a worker thread, then embeds an interactive Tk window (`_open_target_window`) that reuses `_draw_geographic_axes`/`_draw_st_axes` with Tk target-selector + geo/s-t + reconstruction toggles. `▶ Compute Inference` (`_on_compute_inference`) solves **all** blocks with the current prior hyperparameters and scaled real-data noise (building `_shared_bessel` if needed), then pushes each per-block posterior through `build_property_operator(...)` via `assemble_property_posterior(...)` — i.e. the inference is the pushforward of the inversion, reusing cached `_forward_dict` entries and per-block posteriors. Results (`μ_P`, `σ_P`, correlation) are shown via `full_spectrum_viz.plot_property_posterior_summary` in a Tk window (`_open_inference_window`).
- `prior_posterior_tuner_app.py` / `prior_posterior_tuner_remote.py` (**2026-05-29 europa inference routing**): `▶ Compute Inference` now honours the same Local/Europa compute toggle (`_compute_mode_var`) as the single-block posterior solve. `_on_compute_inference` dispatches to `_on_compute_inference_local` (the original all-block pushforward) or `_on_compute_inference_europa`. The europa path writes `tuner_inference_params.json` with `"task": "inference"` (keys: n_basis, s_max, hyper, taus, sigma_var, data_noise_multiplier, n_jobs; no per-block field), submits `python prior_posterior_tuner_remote.py --params-json … --out-npz …` via `europa submit`, polls `europa status`, runs `europa pull`, and loads `tuner_inference_result.npz` (`mu_P`, `sigma_P`, `corr_P`, `names`) into `_open_inference_window`. On the remote side, `main()` reads `block = params.get("block")` (now optional) and, when `task == "inference"`, dispatches to `_run_inference(...)` which rebuilds the deterministic default targets via `_make_default_targets(specs, s_max)` (no target serialization needed), solves every block, assembles the property posterior, and saves the NPZ. `_target_label(idx, target)` mirrors the GUI labelling.
- `prior_posterior_tuner_app.py` / `prior_posterior_tuner_remote.py` (**2026-05-30 all-block posterior computation**): a new `▶▶ Compute All Blocks` button (status label `_lbl_compute_all`, guarded by `_computing_all`, enabled in `_on_build_done`, text toggled in `_on_mode_change`) solves the Bayesian posterior inversion for every `(s,t)` block with the current prior and caches the results so the existing Block dropdown can browse each per-block posterior without recomputing. `_on_compute_all` routes to `_on_compute_all_local` / `_on_compute_all_europa` via `_compute_mode_var`. The local path reuses the all-block solve loop (`build_block_forward` cached in `_forward_dict`, `_build_scaled_data_noise_measure`, `build_block_prior`, `solve_block`), builds a single `PosteriorViewer(posterior_dict, _forward_dict, specs, blocks=…)`, calls `compute_block(block)` per block, and stores it in `_post_viewer`. The europa path writes `tuner_all_params.json` with `"task": "all_blocks"` (no per-block field), submits/polls/pulls, and loads `tuner_all_result.npz` into an `_EuropaDisplayViewer` keyed by every block. The NPZ stores per-block display arrays under prefixed keys `s{S}_t{T}__<field>` plus a `blocks` index array of `"s,t"` strings. `_on_block_change` now redraws the cached posterior (`_post_viewer.is_computed(block)` or `load_cached_block`) instead of forcing a recompute when the block is cached and data mode is real. All-block solves use real data only (synthetic mode remains a per-block workflow). On the remote side, `main()` dispatches `task == "all_blocks"` to `_run_all_blocks(...)`, which solves every block, computes `_compute_posterior_display_data` per block, and saves the prefixed NPZ via `_block_key(block)`.
- `headless/`, `run_paper_inference.py`, `run_paper_sweep.py` (**2026-06-04 headless paper-runner implementation**): non-GUI runner for reproducible paper sweeps. Configs are JSON/TOML (`configs/paper_baseline_flat.toml`, `configs/paper_baseline_weighted.toml`, `configs/sweep_prior_noise_flat.toml`), with public `model_space.covariance_truncation` naming to avoid conflating covariance/operator truncation with a model-space basis. Flat defaults mirror the tuner stream (`L2(dr)`, ordinary Laplacian, k=1, s=3, alpha≈250000); weighted defaults mirror the radial stream (`L2(r^2dr)`, radial Laplacian, k=1e-3, s=3, alpha=100); per-component BCs are explicit and inherited from `_BESSEL_PARAMS`. Single runs write hashed artifact directories under `runs/` with config/provenance, prior/posterior/inference/target figures, arrays, CSV diagnostics, normal-operator spectra, data-fit diagnostics, timings, and optional dense diagnostics. Sweeps expand to concrete JSON sub-run configs, write one JSONL manifest per execution backend (`manifest_local.jsonl`, `manifest_europa.jsonl`), continue after sub-run failures by default, support multi-path axes for shared hyperparameter changes, support `--offset/--limit` slicing for simultaneous backend partitions, and include an europa submit backend with required `status`/`df`/`claim`/`registry` preflight.
- `run_overnight_validation.py`, `submit_overnight_validation_europa.py`, and the `paper_validation_*` / `sweep_validation_*` configs (**2026-06-05 overnight validation bundle**): a bounded europa-ready validation workload intended to shake out implementation issues rather than generate scientific results. It combines rich flat/weighted single runs (figures + reconstructed fields + dense diagnostics), cheap flat/weighted BC-override sentinels, and lean flat/weighted 12-case matrix sweeps over `noise.data_noise_multiplier`, `model_space.covariance_truncation`, and Lebesgue quadrature density. Outputs are redirected to `intervalinf/work/paper_demos_validation_overnight/**` so `.europa.yml` pulls them back. Local calibration in the Python 3.12 `inferences` env: lean `s_max=2`, truncation 30, no-figure runs take about 171 to 173 s and write about 372 KB; a rich flat sentinel (`s_max=6`, figures enabled) completed in about 929 s and wrote about 5.4 MB. The submit helper uses europa preflight checks and wraps the remote command in a release-on-exit shell so the INFERENCES claim is not left hanging on successful completion.
- `utils/prior_calibration.py`, `run_prior_calibration.py`, and `prior_posterior_tuner_app.py` (**2026-06-05 prior-predictive tau calibration**): dedicated calibration workflow for the full-spectrum paper-demo priors. With fixed radial hyperparameters (`k`, `alpha`, Sobolev order, BCs) and actual even-degree real-SH blocks, it samples reconstructed prior fields and calibrates degree-dependent amplitudes `tau_{p,s}` so the requested quantile of `max |delta ln m_p(r,omega)|` matches a physical target amplitude. The default degree shape is `w_s=(1+s)^(-beta)`, with one amplitude scale per component (`vp`, `vs_IC`, `vs_M`, `rho`) and shared `tau` across all `t` at fixed `s`. The Tk prior-posterior tuner now runs this calibration automatically during **Build Prior**, disables manual tau selection, shows the calibrated tau for the selected block degree, synchronizes `k`, `s`, and `alpha` controls across components/all `(s,t)` blocks, and adds a `🗺 Show Prior Maps` action that samples calibrated priors and opens reconstructed full-field maps in the same geographic viewer used for posterior samples. Prior-map reconstruction now uses a separate coarse radial-depth control (`Prior-map depths`, default `64`, minimum `2`) so browsing prior fields is materially faster than the full posterior display path; prior-map viewing now snaps the depth slider to those sampled radii instead of interpolating continuously between them. Outputs from the standalone runner are written under `intervalinf/work/prior_calibration/**`: `tau_by_degree.csv`, `tau_st.csv`, `component_amplitude_summary.csv`, `calibration_samples.npz`, `prior_calibration_summary.png`, and calibrated reconstructed-field sample maps.
- `utils/prior_calibration.py` (**2026-06-08 split-`vs` calibration update**): `vs_IC` and `vs_M` are no longer amplitude-calibrated as independent physical fields. They remain separate model-space covariance blocks and tau lookup keys, but their shared shear-velocity amplitude scale is calibrated from one split-field maximum, `max(max_IC, max_M)`, so the calibration treats them as one physical `vs` object with an outer-core gap. The split `vs` radial reference blocks are normalized before estimating that shared scale, preventing the weighted `s=0` inner-core Neumann zero mode from suppressing the mantle monopole.
- `visualization/model_map_viz.py` / `prior_posterior_tuner_app.py` (**2026-05-30 posterior-mean model maps**): new `visualization/model_map_viz.py` synthesises geographic maps of the **posterior-mean** model recovered by the all-block inversion. `ModelMapViewer(display_dict, *, s_max, earth_radius_km, icb_radius_km, cmb_radius_km)` is built from the cached `{BlockIndex: _PostBlockDisplayData}` (works for both the local `PosteriorViewer` and the europa `_EuropaDisplayViewer` since both expose `is_computed`/`get_display_data`). Key insight: each block's cached `mean_vp`/`mean_vs_IC`/`mean_vs_M`/`mean_rho` arrays ARE that `(s,t)` harmonic's radial coefficient curve, and `sigma_1_mean` is the CMB-topography coefficient. `depth_slice(component, radius_km)` interpolates the per-block radial coefficient at the slice radius (vp/rho on `[0,R]`; vs uses `mean_vs_IC` for `r≤ICB`, `mean_vs_M` for `r≥CMB`, returns `None` in the outer-core gap), sets each via `_set_st_coeff` (same real-SH ordering as `target_kernel_viz`), and expands with pyshtools (`4π`, DH2). `cmb_map()` uses the `sigma_1_mean` coefficients. `_draw_model_map(fig, viewer, component, radius_km, *, plt)` draws into a Cartopy `PlateCarree` GeoAxes with `cfeature.COASTLINE` continent outlines, RdBu_r imshow, gridlines, and a horizontal `fig.colorbar`. In the GUI, a `🗺 Show Model Maps` button (`_btn_maps`/`_lbl_maps`, enabled in `_on_compute_all_done`) opens `_open_model_map_window`: a Tk Toplevel with a component combobox (vp/vs/rho/CMB) and a radius slider (disabled for CMB; depth = R − r shown live; redraw on slider release to avoid expensive SH expansions mid-drag).
- `visualization/posterior_viz.py` / `prior_posterior_tuner_app.py` (**2026-05-30 posterior sample realisations**): added module-level `compute_model_curves(model_vec, specs, *, n_grid=200)` to `posterior_viz.py` — evaluates the radial coefficient curves of ANY model-space vector (e.g. a `GaussianMeasure.sample()`) into a `_PostBlockDisplayData` (mean fields hold the sample's curves, `sigma_1_mean` = scalar σ₁ from `model_vec[1][1]`, uncertainty fields zero-filled), making it drop-in compatible with `ModelMapViewer`. The local all-block worker `_on_compute_all_local` now reads `self._n_samples_var` (a Spinbox near the Compute-All button, default 8), seeds `np.random.seed(0)`, calls `posterior_dict[block].samples(N)` per block, and transposes into `self._posterior_sample_curves` (a list of `{block: _PostBlockDisplayData}`, one dict per realisation — sample index `j` taken from every block gives a coherent full-model realisation under the block-factorised posterior). `_on_show_maps` builds a `realisations` list `[("Mean", engine)] + [("Sample k", engine_k)]` and passes it to `_open_model_map_window`, which now adds a **Realisation** combobox plus `<`/`>` buttons that swap `state["engine"]` (cheap `ModelMapViewer` rebuild), redraw, update the status text, and replace the map title's "posterior mean" with the selected sample label so the change is visibly confirmed. **Local-only**: the europa all-block path returns display data without the posterior covariance, so it sets `self._posterior_sample_curves = []` and the realisation selector shows only "Mean" / "mean only".
- `visualization/posterior_viz.py` / `tests/test_posterior_viz.py` (**2026-05-29 sigma₁ uncertainty fix**): `_compute_posterior_display_data(...)` still estimates radial uncertainty bands by probing the posterior covariance with normalized Gaussian bumps, but the scalar `σ₁` bar now comes from the posterior covariance of the `σ₁` component itself rather than a `specs.sigma_var` fallback. `_draw_posterior_axes(...)` scales that scalar bar by the same `n_sigma` setting used for the radial bands, and the smoke tests now pin both the `σ₁` covariance extraction and the disk-cache round trip.
- `property_targets.py` (**Phase 5**, updated): `extract_st_coeff(coeffs_array, s, t)` — extracts the `(s,t)` real SH coefficient from a pyshtools `(2, lmax+1, lmax+1)` array using the Dahlen & Tromp t-index convention (t=0→zonal, t=2k-1→cosine order k, t=2k→sine order k). `angular_bump_coeffs(sigma_deg, s_max, lmax_grid)` — raw Gaussian angular cap at the north pole via DH grid + `SHGrid.expand(normalization='4pi')`, returns `(2, s_max+1, s_max+1)` array. `normalized_angular_bump_coeffs(sigma_deg, s_max, lmax_grid)` — same cap normalized to unit surface integral before expansion. `rotate_bump_coeffs(coeffs_array, lat_deg, lon_deg)` — pyshtools ZYZ rotation (alpha=lon, beta=colatitude, gamma=0). `radial_bump(r_km, r0_km, sigma_r_km)` — raw Gaussian radial weight. `normalized_radial_bump(r_km, r0_km, sigma_r_km, normalization_r_km)` — radial factor normalized so $\int a(r)r^2dr=1$ on the reference grid. `BulkTarget` (dataclass: param, lat_deg, lon_deg, sigma_ang_deg, r0_km, sigma_r_km). `CMBTarget` (dataclass: lat_deg, lon_deg, sigma_ang_deg). `build_block_property_coeffs(targets, blocks, s_max)` — pre-computes dict{BlockIndex: np.ndarray(N_p)} of unit-surface-integral angular coefficients, with a pole-template cache keyed on `sigma_ang_deg`.
- `visualization/target_kernel_viz.py` (**new**, updated): standalone Matplotlib viewer for target kernels. `TargetKernelViewer` precomputes the normalized angular coefficients `B_{st,i}` via `build_block_property_coeffs(...)`, synthesizes geographic maps either from the full target expansion up to `s_max` or from the model/data-supported block projection only, returns volume-normalized radial profiles `a(r)` for `BulkTarget`, and returns per-block radial kernels `B_{st,i} a(r)` in s-t view. `MatplotlibTargetKernelViewer` provides target selection, geographic ↔ s-t mode switching, and a geographic reconstruction toggle between `s<=smax` and `model blocks`; the displayed kernels now match the normalized separable mapping used by `build_property_operator(...)`.
- `visualization/full_spectrum_viz.py` (**Phase 7**, updated): Four headline visualisation functions. `plot_block_posterior(block, posterior, forward_dict, specs, *, n_grid=200, n_probes=20)` — 5-panel figure ({δvp, δvs-IC, δvs-M, δρ} each on a separate panel + σ₁ scalar); radial uncertainty bands are probed with `n_probes` narrow Gaussian bumps via the posterior covariance operator, and the σ₁ panel now likewise uses the posterior covariance of the scalar `σ₁` component rather than a `specs.sigma_var` fallback; accesses component spaces via `.subspaces` on `HilbertSpaceDirectSum` / `LebesgueSpaceDirectSum` and function domains via `.function_domain` on `Lebesgue`. `plot_cmb_map(blocks, posterior_dict, s_max, *, n_lat=90, n_lon=180)` — synthesises CMB topography from `posterior_dict[block].expectation[1][1][0]` using pyshtools, plots with Cartopy Mollweide (falls back to imshow if Cartopy unavailable). `plot_equatorial_slice(blocks, posterior_dict, s_max, param='vs', *, enabled=False, ...)` — returns `None` immediately when `enabled=False`; synthesises volumetric map at `r_km` when enabled. `plot_property_posterior_summary(mu_P, sigma_P, corr_P, target_names)` — two-panel figure: horizontal bar chart (mean ± 1σ) + correlation heatmap. All functions guard pyshtools/cartopy imports with clear `ImportError`; none call `plt.show()`.
- `tests/test_full_spectrum_utils.py` (**Phases 1–6**, updated): 16 tests — block indexing (3), Phase 2 forward shapes (4), Phase 3 priors (3), Phase 4 posteriors (3), Phase 6 property posterior (3: mean_matches_pushforward, cov_symmetric_psd, reduces_uncertainty).
- `tests/test_property_targets.py` (**Phase 5**, new): 7 tests — `test_angular_bump_coeffs_axisymmetric_at_pole`, `test_rotation_preserves_power`, `test_radial_bump_peak_location`, `test_property_operator_shape`, `test_property_operator_forward_cmb_target`, `test_property_operator_adjoint_consistency`, `test_property_operator_vs_target` (exercises IC/M subdomain adjoint paths).
- `visualization/prior_viz.py` (**Phase 8**, updated): Prior visualisation module. `_PriorComponentData` (dataclass: `r_grid (n_grid,)`, `ref_std (n_grid,)`, `ref_samples (n_samples, n_grid)`, `_eigenvalues (n_basis,)`, `_phi_matrix (n_basis, n_grid)`). `PriorViewer(shared_bessel, specs, *, n_grid=300, n_samples=5, seed=42)` — precomputes reference std (√(Σ_k λ_k φ_k(r)²)) and KL samples at τ=1 for all four radial params; `precompute(progress_cb)` builds phi_matrix from `BesselSobolevInverse.get_eigenfunction(k)(r_grid)` (vectorised); `resample(seed)` redraws samples without touching eigenpairs; `invalidate_bessel(new_shared_bessel)` clears cache; `get_display_data(block, *, tau_vp, tau_vs_IC, tau_vs_M, tau_rho, sigma_var)` returns τ-scaled dict instantly. `_render_prior_figure(display_data, block, specs, *, n_sigma=2.0)` — 5-panel 1×5 matplotlib Figure (δvp, δvs_IC, δvs_M, δρ, σ₁); each radial panel shows zero-mean dashed, ±1σ and ±2σ shaded bands, semi-transparent sample curves. `_draw_prior_axes(axes, display_data, block, specs, *, n_sigma=2.0)` — shared axes-level renderer used by both notebook and standalone viewers. `make_prior_widget(viewer, blocks, specs, *, tau_init, sigma_var_init)` — ipywidgets VBox with block dropdown, four τ FloatSliders, σ_var FloatLogSlider, Precompute + Resample buttons, Output widget; all slider/dropdown changes trigger instant redraw from cache. `MatplotlibPriorViewer(viewer, blocks, specs, plt, Button, Slider, *, tau_init, sigma_var_init, n_sigma=2.0)` — standalone Matplotlib control panel with block-index slider, previous/next buttons, four τ sliders, log10 σ_var slider, Precompute/Resample buttons, left/right keyboard navigation, and `show()` / `save()` methods. CLI entry point: `python visualization/prior_viz.py --precompute`.
- `tests/test_full_spectrum_viz.py` (**Phase 7**, updated): 4 smoke tests — `test_plot_block_posterior_returns_figure` (self-contained: uses `build_block_prior` as proxy posterior, no external data files, runs in ~4 s), `test_plot_block_posterior_uses_posterior_sigma_1_std` (pins σ₁ against posterior covariance rather than the old fallback prior bar), `test_plot_cmb_map_returns_figure` (synthetic `_FakePosterior` with correct expectation nesting), `test_plot_equatorial_slice_gated_off_by_default` (asserts `None` return).
- `tests/test_prior_viz.py` (**Phase 8**, updated): 7 smoke tests — `test_prior_viewer_precompute_runs` (shapes, std≥0), `test_get_display_data_scales_with_tau` (doubling tau_vp doubles std/samples), `test_get_display_data_samples_shape` (r/mean/std/samples keys, sigma_1 float std), `test_prior_viewer_resample_changes_samples` (different seed → different samples; ref_std unchanged), `test_render_prior_figure_returns_figure` (Figure with 5 axes), `test_make_prior_widget_returns_widget` (ipywidgets Widget), `test_matplotlib_prior_viewer_precompute_redraws` (standalone Matplotlib control panel precomputes and renders the first block with Agg backend).
- `example.ipynb` (**Phases 1–8**, updated): spectral inventory → data visualisation → forward operators → priors → posteriors → property operators + per-block property vector printout → property posterior assembly ($\mu_P$, $C_P$), mean±std per target, correlation heatmap → **Phase 7**: import `full_spectrum_viz`, Figure 1 (per-block radial posterior s=2,t=0), Figure 2 (CMB topography map), Figure 3 (equatorial slice, disabled), Figure 4 (property posterior summary) → **Phase 8**: Phase 3.5 markdown + 2 code cells (PriorViewer create+precompute; make_prior_widget display).
...
│   ├── covariance.py     SpectralCovarianceOperator, MixtureCovarianceOperator
...
LinearOperator               →   SpectralOperator, Gradient, BesselSobolev,
                                 BesselSobolevInverse, SpectralCovarianceOperator,
                                 MixtureCovarianceOperator, SOLAOperator
...
| `LinearOperator` | `SpectralOperator`, `Gradient`, `BesselSobolev`, `BesselSobolevInverse`, `SpectralCovarianceOperator`, `MixtureCovarianceOperator`, `SOLAOperator` | All inherit `@` composition, `.adjoint`, `.dual`, `.matrix()` from pygeoinf |
...
- **Mathematical space:** $L^2([a,b])$ with inner product $\langle u, v \rangle = \int_a^b u(x)v(x)\,dx$ (plain/unweighted). For a *weighted* inner product use `WeightedLebesgue` (below).
- **Constructor:** `Lebesgue(dim, function_domain, /, *, basis=None, integration_config=None, parallel_config=None)`
...
**Phase 2 inner-product fast path:** `_continuous_l2_inner_product` now builds a `RepresentationSpec(kind='fixed_grid', method='uniform', ...)` from the space domain and `integration_npoints`. For Simpson/trapezoid rules it obtains `u.materialize(spec)` and `v.materialize(spec)`, multiplies the cached arrays, and integrates them with the same scipy quadrature rule. Adaptive/quad methods keep the original `(u * v).integrate(...)` path.
...
**Class: `WeightedLebesgue`** (`spaces/weighted_lebesgue.py`)
- **Base:** `pygeoinf.MassWeightedHilbertSpace`
- **Mathematical space:** $L^2([a,b]; w)$ with $\langle u, v\rangle_w = \int u v w\,dx$, realized as a mass-weighted plain `Lebesgue`: mass operator $M=(\times w)$, inverse $M^{-1}=(\times 1/w)$, so $\langle u,v\rangle_w=\langle Mu,v\rangle_{L^2}$.
- **Constructor:** `WeightedLebesgue(dim, function_domain, weight, *, inverse_weight=None, basis=None, integration_config=None, parallel_config=None)`
- **Why it exists:** the SINGLE weighting mechanism (replaces removed `Lebesgue(weight=)`). Because the weight lives in the mass operator, the Riesz maps `to_dual`/`from_dual` are weight-consistent (`to_dual(f)` kernel $=Mf=w f$; `from_dual` applies $M^{-1}$), which makes operator adjoints (SOLA, Bessel) correct on weighted domains. Stores the weight as `_weight_fn` (NOT `_weight`) to avoid the `LinearFormKernel` legacy pickup. `weight`/`function_domain`/`integration`/`parallel` accessors delegate to the underlying space. Tests: `tests/spaces/test_weighted_lebesgue.py`. Singularity caveat: `1/w` blows up where `w→0` (e.g. `r²` at 0) — use `[ε,R]` or a regularized `inverse_weight`. **Design:** memory `weighting-via-massweighted-design`.
...
- **Fast path** (when L is Laplacian with standard BCs **and the domain is unweighted**): Uses DST/DCT/DFT; scales coefficients by $(k^2 + \lambda_i)^{s/2}$
- **Slow path** (Robin BCs, or any **weighted** domain): Uses numerical integration
- **Weighted (e.g. radial r²) domains:** When the domain is a `MassWeightedHilbertSpace` (e.g. `WeightedLebesgue` with `w=r²`), the slow-path spectral projection uses the *space* inner product $\langle\phi_i,f\rangle=\int\phi_i f\,w\,dr$ (passed as `compute_spectral_coefficients_slow(..., inner_product=domain.inner_product)`). Combined with a `RadialLaplacian` (whose eigenfunctions are $r^2$-orthonormal), this makes `BesselSobolev`/`BesselSobolevInverse` a correct, self-adjoint covariance w.r.t. $\langle f,g\rangle=\int fg\,r^2\,dr$ — **no separate radial-Bessel class is needed**, only a radial `L`. The generic DST/DCT fast path is automatically disabled for mass-weighted domains (`_can_use_fast_transforms` requires `not isinstance(domain, MassWeightedHilbertSpace)`), since those transforms compute unweighted coefficients. Covered by `tests/operators/test_weighted_bessel_radial.py`. (The former `Lebesgue(weight=)` mechanism has been removed; use `WeightedLebesgue`.)
- **Radial Dirichlet fast path:** when `L` is a spectral `RadialLaplacian` with Dirichlet BCs and `ell=0`, `BesselSobolev{,Inverse}` use a dedicated DST fast path (`self._radial_dirichlet_fast`, checked first in `_apply`) via the substitution $u(r)=r\,f(r)$: the weighted radial projection of $f$ equals the flat sine (Dirichlet) projection of $u$ (same $c_n=\sqrt{2/L}$, $\lambda_n=(n\pi/L)^2$), so the existing flat DST machinery runs on $u$ and the result is reconstructed with the radial eigenfunctions (module helpers `_radial_dirichlet_fast_eligible`, `_apply_radial_fast_impl`). Neumann/mixed/Robin radial spectra and all `ell>0` radial spectra use the slow weighted spectral projection path. Verified fast==slow and against eigenfunction scaling/self-adjointness.

**Class: `BesselSobolevInverse`**
- **Base:** `pygeoinf.LinearOperator`
- **Mathematical operator:** $(k^2 I - \Delta)^{-s/2}$
- Same structure as `BesselSobolev` but scales by $(k^2 + \lambda_i)^{-s/2}$ (including the weighted-domain / radial behaviour above)
...
#### `operators/covariance.py` — `SpectralCovarianceOperator`, `MixtureCovarianceOperator`

**Purpose:** Function-space covariance templates for exploratory priors. These operators keep priors as `pygeoinf.LinearOperator` objects on `Lebesgue`/`Sobolev` domains; dense matrices are only diagnostic projections such as $G C G^*$.

**Class: `SpectralCovarianceOperator`**
- **Base:** `SpectralOperator`
- **Mathematical operator:** $C f = \sum_i \lambda_i \langle f, \phi_i \rangle \phi_i$ using eigenfunctions from a reference spectral operator and a non-negative eigenvalue law.
- **Constructor:** `SpectralCovarianceOperator(domain, reference_operator, eigenvalue_law, *, dofs=None, integration_config=None)`
- Validates finite non-negative covariance eigenvalues and exposes `trace_estimate()` and `effective_rank()` diagnostics.
- Supplies its own self-adjoint mapping so it is suitable as a `GaussianMeasure(covariance=...)` backend on basis-free function spaces.

**Class: `MixtureCovarianceOperator`**
- **Base:** `pygeoinf.LinearOperator`
- **Mathematical operator:** $C = \sum_j w_j C_j$ with $w_j \geq 0$ and all $C_j$ on the same Hilbert space.
- Designed for smooth-plus-rough priors where a broad rough component prevents the prior from assigning near-zero variance to data-visible directions.

Normal-mode exploration helpers live in `demos/old_demos/paper_demos/utils/prior_family_exploration.py`: `make_bessel_covariance`, `make_power_law_covariance`, `make_mixture_covariance`, `build_block_prior_from_component_covariances`, `data_space_covariance_operator`, `DataSpaceCoverage`, and information/contraction spectrum utilities. These helpers explicitly distinguish the infinite-dimensional function-space problem from finite data-space diagnostics.
...
- Computes eigenvalues of
  $-\partial_r^2 - (2/r)\partial_r + \ell(\ell+1)/r^2$
  for various BCs on $(0,R)$ or $(a,b)$
- Supports: Dirichlet-Dirichlet, Dirichlet-Neumann, Neumann-Dirichlet, Neumann-Neumann at domain endpoints
- For `ell=0`, preserves the existing analytical/special providers: analytical DD and regularity-Dirichlet eigenvalues, numerical root-finding (via `RobinRootFinder`) for NN and mixed
- For `ell>0`, delegates to `GeneralRadialLaplacianModeSolver`, which solves spherical-Bessel boundary determinant equations with `scipy.optimize.brentq`
...
- For finite-difference application, includes the centrifugal term $\ell(\ell+1)/r^2$ on the diagonal; for spectral application, uses `RadialLaplacianSpectrumProvider`
...
| `GeneralRadialLaplacianProvider` | $(0, R)$ or $(a, b)$ | D/N/mixed for `ell>0` | Numerically normalized spherical-Bessel modes: regular $j_\ell(kr)$ on $(0,R)$; $A j_\ell(kr)+B y_\ell(kr)$ on shells |

`GeneralRadialLaplacianModeSolver` underpins the general provider. It caches roots and mode coefficients, uses `scipy.special.spherical_jn` / `spherical_yn` plus derivative helpers for boundary equations, and normalizes modes in the weighted radial inner product $\int_a^b \phi_i(r)\phi_j(r)r^2dr$.
