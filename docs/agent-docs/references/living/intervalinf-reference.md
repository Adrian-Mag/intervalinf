...
- `tests/test_full_spectrum_viz.py` (**Phase 7**, new): 3 smoke tests — `test_plot_block_posterior_returns_figure` (self-contained: uses `build_block_prior` as proxy posterior, no external data files, runs in ~4 s), `test_plot_cmb_map_returns_figure` (synthetic `_FakePosterior` with correct expectation nesting), `test_plot_equatorial_slice_gated_off_by_default` (asserts `None` return).
- `example.ipynb` (**Phases 1–7**, updated): spectral inventory → data visualisation → forward operators → priors → posteriors → property operators + per-block property vector printout → property posterior assembly ($\mu_P$, $C_P$), mean±std per target, correlation heatmap → **Phase 7**: import `full_spectrum_viz`, Figure 1 (per-block radial posterior s=2,t=0), Figure 2 (CMB topography map), Figure 3 (equatorial slice, disabled), Figure 4 (property posterior summary).
