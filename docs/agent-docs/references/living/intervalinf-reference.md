...
- **Radial Dirichlet fast path:** when `L` is a spectral `RadialLaplacian` with Dirichlet BCs, `BesselSobolev{,Inverse}` use a dedicated DST fast path (`self._radial_dirichlet_fast`, checked first in `_apply`) via the substitution $u(r)=r\,f(r)$: the weighted radial projection of $f$ equals the flat sine (Dirichlet) projection of $u$ (same $c_n=\sqrt{2/L}$, $\lambda_n=(n\pi/L)^2$), so the existing flat DST machinery runs on $u$ and the result is reconstructed with the radial eigenfunctions (module helpers `_radial_dirichlet_fast_eligible`, `_apply_radial_fast_impl`). Neumann/mixed/Robin radial spectra are transcendental → slow path. Verified fast==slow and against eigenfunction scaling/self-adjointness.
