...
# ---------------------------------------------------------------------------
# Topographic kernel matrix (standalone function)
# ---------------------------------------------------------------------------
...
def build_topo_matrix(
    ...
    catalog : NormalModeKernelCatalog
        Kernel data source.
    observations : list of tuple (mode_id, s, t)
        Observation list in the same order as the data space.
    ...
    """
    keys = observations[:n_obs] if n_obs is not None else observations
    out: List[float] = []
    for mode_id, s, _ in keys:
        v = catalog.get_topo_layer_value(mode_id, s, prem_layer)
        out.append(v)
    return np.array(out)
