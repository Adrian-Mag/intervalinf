...
-----------
  ...
  Over-smooth  : s=6, ls=150, ov=10^1.5, mean_offset=0  (long correlation length)
  Under-smooth : s=3, ls=10,  ov=10^1.5, mean_offset=0  (rough spectrum)
  Shifted-mean : s=6, ls=30,  ov=10^1.5, mean_offset=+3 (prior mean far from truth)
...
----------------------
  ...
  unc_reduction    1 - mean_post_std / mean_prior_std   (uncertainty reduction)
  data_misfit      ||G(m̃) - d̃||  (how well posterior mean fits data)
  min_eig_cov      Min eigenvalue of property covariance matrix (stability check)
...
from kernel_utils import (
    SensitivityKernelCatalog, SensitivityKernelProvider,
    DepthCoordinateSystem, EARTH_RADIUS_KM,
)
...
parallel_cfg = ParallelConfig(enabled=True, n_jobs=30)
...
M_vp  = Lebesgue(N, function_domain, basis='ND',
...
M_rho   = Lebesgue(N, function_domain, basis='ND',
                   integration_config=Lebesgue_integration_cfg.inner_product,
                   parallel_config=parallel_cfg)
...
G_vp    = SOLAOperator(M_vp,    D, vp_kernel_function_provider,  integration_config=sola_integration_cfg)
G_rho   = SOLAOperator(M_rho,   D, rho_kernel_function_provider, integration_config=sola_integration_cfg)
...
ran_array_vp    = np.zeros(N); ran_array_vp[:10]    = np.random.RandomState(42).uniform(-1, 1, 10)
ran_array_vs_IC = np.zeros(N); ran_array_vs_IC[:10] = np.random.RandomState(24).uniform(-1, 1, 10)
ran_array_vs_M  = np.zeros(N); ran_array_vs_M[:10]  = np.random.RandomState(84).uniform(-1, 1, 10)
ran_array_rho   = np.zeros(N); ran_array_rho[:10]   = np.random.RandomState(12).uniform(-1, 1, 10)
...
np.random.seed(42)
d_tilde     = [sub + np.random.normal(0, noise_level, sub.shape)
               for sub in d_bar]
...
L_vs_IC = Laplacian(M_vs_IC, bcs_vs_IC, alpha_vs, method='spectral', dofs=100,
                    integration_config=laplacian_integration_cfg, n_samples=2048)
L_vs_M  = Laplacian(M_vs_M,  bcs_vs_M,  alpha_vs, method='spectral', dofs=100,
                    integration_config=laplacian_integration_cfg, n_samples=2048)
C_0_vs_IC = BesselSobolevInverse(M_vs_IC, M_vs_IC, k_vs, s_vs, L_vs_IC,
                                  dofs=512, n_samples=2048, use_fast_transforms=True,
                                  integration_config=bessel_sobolev_integration_cfg)
C_0_vs_M  = BesselSobolevInverse(M_vs_M,  M_vs_M,  k_vs, s_vs, L_vs_M,
                                  dofs=512, n_samples=2048, use_fast_transforms=True,
                                  integration_config=bessel_sobolev_integration_cfg)
...
L_rho = Laplacian(M_rho, bcs_rho, alpha_rho, method='spectral', dofs=100,
                  integration_config=laplacian_integration_cfg, n_samples=2048)
C_0_rho = BesselSobolevInverse(M_rho, M_rho, k_rho, s_rho, L_rho,
                                dofs=512, n_samples=2048, use_fast_transforms=True,
                                integration_config=bessel_sobolev_integration_cfg)
...
M_prior_vs_IC  = GaussianMeasure(covariance=C_0_vs_IC, expectation=m_0_vs_IC, sample=sampler_vs_IC.sample)
sampler_vs_M   = KLSampler(C_0_vs_M,  mean=m_0_vs_M,  n_modes=100)
M_prior_vs_M   = GaussianMeasure(covariance=C_0_vs_M,  expectation=m_0_vs_M,  sample=sampler_vs_M.sample)
...
M_prior_sigma_0 = GaussianMeasure.from_covariance_matrix(M_sigma_0, np.array([[10]]), expectation=np.array([0.0]))
M_prior_sigma_1 = GaussianMeasure.from_covariance_matrix(M_sigma_1, np.array([[10]]), expectation=np.array([0.0]))
...
def build_vp_prior(s, length_scale, overall_variance, mean_offset=0.0, n_kl=100):
    ...
    L = Laplacian(M_vp, bcs_vp, alpha, method='spectral', dofs=100,
                  integration_config=laplacian_integration_cfg, n_samples=2048)
    ...
    m_0 = Function(M_vp, evaluate_callable=lambda x, _off=mean_offset: _off * np.ones_like(x))
    sampler = KLSampler(C, mean=m_0, n_modes=n_kl)
    return GaussianMeasure(covariance=C, expectation=m_0, sample=sampler.sample)
...
# overall_variance governs the amplitude; we keep it fixed at 10^1.5 across cases
# so the only driver of change is the _shape_ of the prior, not its overall scale.
#
# Over-smooth uses s=12 (doubled Sobolev order) rather than a longer length_scale.
# Increasing length_scale also increases alpha = ls^2 * k^2, collapsing the
# BesselSobolev eigenvalues and making the prior over-constrained (100x tighter).
# Increasing s instead leaves alpha similar (~675 vs ~605 for reference) and only
# changes the rate at which high-frequency components are penalised.
#
# For 'Shifted-mean' the offset of +3 is chosen so that the prior mean is far
# enough from the truth to demonstrate mean-bias, while not being so extreme that
# the Cholesky solver cannot converge.

PRIOR_CASES = {
    "Reference":    dict(s=6.0,  length_scale=30, overall_variance=10**1.5, mean_offset=0.0),
    "Over-smooth":  dict(s=12.0, length_scale=30, overall_variance=10**1.5, mean_offset=0.0),
    "Under-smooth": dict(s=3.0,  length_scale=10, overall_variance=10**1.5, mean_offset=0.0),
    "Shifted-mean": dict(s=6.0,  length_scale=30, overall_variance=10**1.5, mean_offset=3.0),
}
...
for label, cfg in PRIOR_CASES.items():
    ...
    msg = (
        ...
        f"  RMSE={rmse:.4f}  MAE={mae:.4f}  Coverage(2σ)={cover:.0%}\n"
        f"  Mean post σ={mean_post_std:.4f}  Mean prior σ={mean_prior_std:.4f}\n"
        f"  Uncertainty reduction={unc_reduction:.1%}\n"
        f"  Data misfit={data_misfit:.4f}  min eigenvalue={min_eig:.2e}\n"
    )
    print(msg)
