...
def plot_block_posterior(
    ...
    *,
    ...
    - Panel 5: scalar sigma_1 (CMB topography) shown as a horizontal line
            (posterior mean) with a ±1σ error bar derived from the posterior
            covariance of the sigma_1 component.
    ...
    The scalar ``sigma_1`` uncertainty is extracted directly from the
    posterior covariance by applying it to the unit vector in the ``σ₁`` slot.
    ...
    sigma_1_input = [[zero_vp, [zero_vs_IC, zero_vs_M], zero_rho], [zeros1, np.array([1.0])]]
    sigma_1_cov_out = posterior.covariance(sigma_1_input)
    sigma_1_var = float(np.asarray(sigma_1_cov_out[1][1], dtype=float).reshape(-1)[0])
    sigma_1_std = float(np.sqrt(max(0.0, sigma_1_var)))

    # ── Plot ─────────────────────────────────────────────────────────────────
    # Layout: 4 panels — {δvp, δvs (combined), δρ, σ₁}
    ...
    # Panel 1: vs — combined inner core and mantle with outer core shaded
    # Combine the two vs grids into a single continuous grid
    ...
    # Plot combined vs
    axes[1].plot(mean_vs_combined, r_vs_combined, color='forestgreen', lw=1.5, label='mean')
    ...
    # Shade outer core region (ICB to CMB)
    axes[1].axhspan(ICB, CMB, color='gray', alpha=0.3, label='outer core')
    axes[1].set_ylabel('radius (km)')
    axes[1].set_xlabel('δvs')
    axes[1].legend(fontsize=7)

    # Panel 2: rho
    _plot_radial_panel(axes[2], r_rho, mean_rho, probe_r_rho, std_rho, 'δρ', color='goldenrod')

    # Panel 3: sigma_1 scalar (CMB topography) — posterior ±1σ
    axes[3].axhline(sigma_1_mean, color='firebrick', lw=2, label=f'mean = {sigma_1_mean:.3f}')
    axes[3].errorbar(
        x=0.5, y=sigma_1_mean, yerr=sigma_1_std,
        fmt='o', color='firebrick', capsize=6, label=f'±σ_post = {sigma_1_std:.2f}',
    )
    axes[3].set_xlim(0, 1)
    axes[3].set_xticks([])
    axes[3].set_ylabel('σ₁ (CMB topo)')
    axes[3].set_xlabel('σ₁')
...
def main() -> None:
    """Explain how to use this helper module when run directly."""
    print(
        "full_spectrum_viz.py defines plotting helper functions; it does not "
        "run a viewer by itself. It lives in visualization/ because the "
        "folder is organized by visualization responsibility, not only by "
        "whether a module is an app.\n\n"
        "It expects objects built by the full-spectrum workflow, such as "
        "posterior_dict, forward_dict, specs, and property posterior arrays. "
        "Use it from example.ipynb after those objects exist, for example:\n\n"
        "    from full_spectrum_viz import plot_block_posterior\n"
        "    fig = plot_block_posterior(block, posterior_dict[block], "
        "forward_dict, specs)\n"
        "    fig.savefig('block_posterior.png', dpi=150)\n"
        "    plt.show()\n\n"
        "For standalone interactive visual tools, run one of:\n"
